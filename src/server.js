...
//   GET  /               → serves public/index.html
//   POST /api/decode     → { replaydata } → fully decoded replay + pipeline stats
//   GET  /api/latest     → newest replays from bonk.io (offset=0, no anchor)
//   GET  /api/fetch?startingFrom=N
//                        → best-effort: anchors pagination at replay id N.
//                          NOT a guaranteed exact-id lookup — bonk.io's API
//                          doesn't appear to expose one (see docs). This
//                          returns whatever batch the anchor yields.
//
// Run:  node src/server.js     (PORT defaults to 3000; Railway injects PORT)
//
// It reuses the project's real codec (codec.js) and API client (api.js), so what
...
// ─── Route: GET /api/latest ──────────────────────────────────────────────────
//
// Under the real anchor+offset model, "give me the newest replays" is just
// offset=0 with no anchor — a single call, no discovery/probing needed.

async function handleLatest(res) {
    try {
        const result = await fetchReplays(0, null);
        ...
        return sendJson(res, 200, {
            ok:      true,
            maxId:   replays[0]?.id ?? null, // newest-first feed → first item is newest
            count:   replays.length,
            replays: replays.map(slim),
        });
    } catch (err) {
...
// ─── Route: GET /api/fetch?startingFrom=N ────────────────────────────────────
//
// Best-effort anchor lookup. bonk.io's API does not appear to expose a true
// "get exactly replay id X" endpoint — startingFrom anchors a pagination
// sequence, it isn't confirmed to jump precisely to that id. Treat this route
// as "browse from around id N", not as a guaranteed exact match.

async function handleFetch(res, url) {
    const startingFrom = parseInt(url.searchParams.get("startingFrom") ?? "", 10);
    if (!Number.isFinite(startingFrom) || startingFrom < 0) {
        return sendJson(res, 400, { ok: false, error: "startingFrom must be a positive integer." });
    }

    const result = await fetchReplays(0, startingFrom);
    switch (result.status) {
        case "ok":
            return sendJson(res, 200, {
                ok: true, startingFrom, count: result.replays.length, replays: result.replays.map(slim),
                note: "Anchored around this id — bonk.io doesn't guarantee an exact-id match, so check the returned ids.",
            });
        case "caught_up":
            return sendJson(res, 200, {
                ok: true, startingFrom, count: 0, replays: [],
                note: `No replays anchored at ${startingFrom} — it may be too old, already expired, or not a valid replay id.`,
            });
        case "ratelimit":
            return sendJson(res, 429, { ok: false, error: "bonk.io rate-limited the request. Wait a minute and try again." });
        default:
